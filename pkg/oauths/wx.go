...
func (w *wxAuth) GetToken(code string) (*AccessToken, error) {
	...
	sc, body, err := cli.Get(context.Background(), w.baseUrl+"/sns/oauth2/access_token?"+p.Encode(), nil)
	if err != nil {
		return nil, err
...
func (w *wxAuth) RefreshToken(refreshToken string) (*RefreshToken, error) {
	...
	sc, body, err := cli.Get(context.Background(), w.baseUrl+"/sns/oauth2/refresh_token?"+p.Encode(), nil)
	if err != nil {
		return nil, err
	} else if !nets.Is2xx(sc) {
		return nil, errors.New("bad status code: " + strconv.Itoa(sc))
...
func (w *wxAuth) GetUserInfo(openid string, accessToken string) (*UserInfo, error) {
	...
	sc, body, err := cli.Get(context.Background(), w.baseUrl+"/sns/userinfo?"+p.Encode(), nil)
	if err != nil {
		return nil, err
	} else if !nets.Is2xx(sc) {
		return nil, errors.New("bad status code: " + strconv.Itoa(sc))
